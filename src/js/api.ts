...
const createUserWord = async (userWordObj: UserWord) => {
  const rawResponse = await fetch(`${users}/${userWordObj.userId}/words/${userWordObj.wordId}`, {
    method: 'POST',
    ...
    body: JSON.stringify(userWordObj.word)
  });
  const content = await rawResponse.json();
